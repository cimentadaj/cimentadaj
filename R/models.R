...
#' @param family_models The link family to use. See ?family
#'
...
#' # Don't have time
#'
models <- function(dv, covariates, data, family_models = "gaussian") {
  dv <- paste(dv, "~ 1")
  ...
  results <- lapply(formulas, function(o) with(data, svyglm(o, family = eval(family_models)))[[1]])
  return(results)
}
